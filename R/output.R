...
if (baseline.method=="BAF"){
   bin.ratio.cols<-rep("#D3D3D3",length(bin.ratio))
    for (i in seq(length(object@seg$summary$seg.median))) {
     ...
     }else if (object@seg$summary$seg.median[i] < -0.1){ 
       bin.ratio.cols[as.vector(seqnames(object@anno@bins))==object@seg$summary$chrom[i] & object@seg$summary$loc.start[i] <
...
setGeneric("CNV.genomeplot_vglGROUP", function(object, ...) {
  standardGeneric("CNV.genomeplot_vglGROUP")
})
setMethod("CNV.genomeplot_vglGROUP", signature(object = "CNV.analysis"), function(object, 
                                                                                  chr = "all", chrX = TRUE, chrY = TRUE, centromere = TRUE, detail = TRUE, 
                                                                                  main = NULL, ylim = c(-1.25, 1.25), set_par = TRUE, cols = c("lightgrey", 
                                                                                                                                               "lightgrey", "lightgrey", "lightgrey", "lightgrey"),diag.group=NA) {
  # if(length(object@fit) == 0) stop('fit unavailable, run CNV.fit')
  if (length(object@bin) == 0) 
    stop("bin unavailable, run CNV.bin")
  # if(length(object@detail) == 0) stop('bin unavailable, run CNV.detail')
  ...
  for (i in seq(length(object@seg$summary$seg.median))) {
    if (object@seg$summary$seg.median[i] > 0.1) { 
      bin.ratio.cols[as.vector(seqnames(object@anno@bins))==object@seg$summary$chrom[i] & object@seg$summary$loc.start[i] < values(object@anno@bins)$midpoint &object@seg$summary$loc.end[i] > values(object@anno@bins)$midpoint ]<-"#00ff00"
    }else if (object@seg$summary$seg.median[i] < -0.1){ 
      bin.ratio.cols[as.vector(seqnames(object@anno@bins))==object@seg$summary$chrom[i] & object@seg$summary$loc.start[i] < values(object@anno@bins)$midpoint &object@seg$summary$loc.end[i] > values(object@anno@bins)$midpoint ]<-"#ff0000"
    }else{bin.ratio.cols[as.vector(seqnames(object@anno@bins))==object@seg$summary$chrom[i] & object@seg$summary$loc.start[i] < values(object@anno@bins)$midpoint &object@seg$summary$loc.end[i] > values(object@anno@bins)$midpoint ]<-"#D3D3D3"
    }
  }   
